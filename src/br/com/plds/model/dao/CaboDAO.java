...
public class CaboDAO {
	...
	public ArrayList<Cabo> getTipos() throws ClassNotFoundException,
			SQLException {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(
					"SELECT tipo FROM cabo");
			ArrayList<Cabo> tipos = new ArrayList<>();
	...
	public boolean atribuir(Cabo m) throws ClassNotFoundException,
			SQLException {
		...
		try {
			...
			con = ConexaoDAO.getConnection();
			st = con.prepareStatement("INSERT INTO atr_cabo (num_serie,tipo,fabricante,mat_tecnico,status) VALUES(?,?,?,?,'ATRIBUIDO')");
	...
	public boolean cadastrar(Cabo m) throws ClassNotFoundException,
			SQLException {
		...
		try {
			...
			con = ConexaoDAO.getConnection();
			st = con.prepareStatement("INSERT INTO cabo (tipo,fabricante) VALUES(?,?)");
	...
	public ArrayList<Cabo> getCaboAtribuidosPorTecnico(String mat)
			...
			ResultSet rs = con
					...
					.executeQuery(
							"SELECT id_atribuicao,num_serie,tipo,fabricante,mat_tecnico,data_atribuicao,data_baixado,num_circuito"
									+ ",rat_frente,rat_verso,status FROM atr_cabo where mat_tecnico='"
	...
	public boolean darBaixa(Cabo m) throws ClassNotFoundException,
			SQLException {
		...
		try {
			...
			con = ConexaoDAO.getConnection();
			st = con.prepareStatement("UPDATE atr_cabo SET data_baixado = CURRENT_TIMESTAMP,num_circuito = ?,"
